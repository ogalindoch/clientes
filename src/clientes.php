...
class clientes implements \euroglas\eurorest\restModuleInterface
{
	...
	public function importaClientes() {
		...
		{
			if($DEBUG) print("Importando de {$origen} desde la variable 'csv'".PHP_EOL);
	...
	public function importaMovimientos()
	{
		...
		set_time_limit(120); // Este proceso puede tardar un largo tiempo
		...
		//
		// Los datos pueden venir en un parametro xml ó xmlfile (no ambos)
		// por eso usamos 'else', y no hacemos 'if's separados.
		//
		if( !empty($_POST['xml']) )
		{
			// Importa desde una variable POST
			if($DEBUG) print("Importando de {$origen} desde una variable POST".PHP_EOL);

			$datosXML = simplexml_load_string($_POST['xml']);
			$this->importaMovimientosSaeXml($datosXML);
		}
		else if( !empty($_FILES['xmlfile']) )
		{
			if($DEBUG) print("Importando de {$origen} desde un archivo".PHP_EOL);
			...
			die(json_encode(
				$this->reportaErrorUsandoHateoas(
					...
					'Necesita incluir un parametro xml (datos en-linea) ó xmlfile (datos en archivo).'
				))
			);
		}
	}
